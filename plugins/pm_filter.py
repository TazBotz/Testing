    ...
    if message.chat.id != SUPPORT_CHAT_ID:
        ...
        if total_results == 0:
            ...
            return await message.reply_text(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton(text='🔍Sᴇᴀʀᴄʜ Hᴇʀᴇ🔍', url=GRP_LNK)]])
            )
...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            ...
            InlineKeyboardButton("Qᴜᴀʟɪᴛɪᴇꜱ", callback_data=f"qualities#{key}") 
        ])
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await query.answer("𝑰 𝒍𝒊𝒌𝒆 𝒚𝒐𝒖𝒓 𝒔𝒎𝒂𝒓𝒕𝒏𝒆𝒔𝒔 𝒃𝒖𝒕 𝒅𝒐𝒏'𝒕 𝒃𝒆 𝒐𝒗𝒆𝒓 𝒔𝒎𝒂𝒓𝒕...😉\n\n𝑱𝒐𝒊𝒏 𝒃𝒂𝒄𝒌𝒖𝒑 𝒄𝒉𝒂𝒏𝒏𝒆𝒍 𝒇𝒊𝒓𝒔𝒕 & 𝒕𝒓𝒚 𝒂𝒈𝒂𝒊𝒏.", show_alert=True)
            return
        ...
        buttons = [[
            ...
            InlineKeyboardButton('🏠 Gᴏ Hᴏᴍᴇ', callback_data='start'),
        ...
        buttons = [[
            InlineKeyboardButton('Lᴀᴢʏ Sᴜᴘᴘᴏʀᴛ Gʀᴏᴜᴘ', url=f'https://t.me/+7joy3I3RJt02YmY9')
        ],[
        ...
        await query.message.edit_text(
    ...
    elif query.data == "rls_dsc":
            btn = [[
                    InlineKeyboardButton('📄 Rᴇᴀᴅ Rᴜʟᴇꜱ', url=f'https://telegra.ph/Hᴏᴡ-Tᴏ-Gᴇᴛ-Mᴏᴠɪᴇꜱ--Sᴇʀɪᴇꜱ-Fʀᴏᴍ-Bᴏᴛ-06-28'),
                    InlineKeyboardButton('⚠️ Dɪꜱᴄʟᴀɪᴍᴇʀ', callback_data='dsc')
                  ],[
                    InlineKeyboardButton("🚪 Bᴀᴄᴋ", callback_data="about")
                  ]]
            await client.edit_message_media(
            ...
            filbtn = [[
                       InlineKeyboardButton("🚪 Bᴀᴄᴋ", callback_data="rls_dsc")
                     ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
            ...
            btn = [[
                    InlineKeyboardButton("⟸ Bᴀᴄᴋ", callback_data="start"),
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url=CHNL_LNK)
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        if len(message.text) < 100:
            search = message.text
            m=await message.reply_sticker("CAACAgUAAxkBAAEKLy1k8X4av59ToQawSHpcjVu2wyAcBwACtQkAAk9FgFdr5af3zLsITzAE")
            search = search.lower()
            find = search.split(" ")
            search = ""
    ...
    if imdb:
        ...
        if settings["button"]:
            cap = f"<b>Rᴇꜱᴜʟᴛꜱ Fᴏʀ ☞ {search}\n\nRᴇǫᴜᴇsᴛᴇᴅ Bʏ ☞ {message.from_user.mention}\n\nRᴇsᴜʟᴛ ɪɴ ☞ {remaining_seconds} Sᴇᴄᴏɴᴅs\n\nPᴏᴡᴇʀᴇᴅ Bʏ ☞ {message.chat.title} \n\nTʜɪs ᴍᴇssᴀɢᴇ ᴡɪʟʟ ʙᴇ ᴀᴜᴛᴏ ᴅᴇʟᴇᴛᴇᴅ ᴀꜰᴛᴇʀ 5 ᴍɪɴᴜᴛᴇ.🗑️\n\n</b>"
        else:
